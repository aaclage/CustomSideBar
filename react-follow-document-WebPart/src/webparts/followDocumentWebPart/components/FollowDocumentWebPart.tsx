...
export default class FollowDocumentWebPart extends React.Component<IFollowDocumentWebPartProps, IFollowDocumentWebPartState> {
    ...
    private getFollowDocuments = async (siteId: string, listId: string): Promise<any> => {
        const GraphService: Graph = new Graph();
        let graphData: any = [];
        graphData = await GraphService.getGraphContent(`https://graph.microsoft.com/v1.0/sites/${siteId}/Lists/${listId}/items?$select=id,fields&expand=fields(select=ItemId,ListId,SiteId,webId,Title,Url,ServerUrlProgid,IconUrl,File_x0020_Type.progid,ItemUniqueId)&$filter=fields/ItemId gt -1`, this.props.context);
                ...
                <div className={styles.spinnerLoading}>
                    ...
                    {(!this.state.visible) &&
                        <Stack tokens={stackTokens}>
                            ...
                            <SearchBox style={{ width: "80%" }} placeholder="Search Document" onChanged={checkSearchDrive} onSearch={checkSearchDrive} onClear={checkClear} />
                        </Stack>
                    }
                </div>
