...
export default class OneDriveFinder extends React.Component<IOneDriveFinderProps, IOneDriveFinderState> {
  ...
  public render(): React.ReactElement<IOneDriveFinderProps> {
    ...
    const { siteID, itemID, pageSize, breadcrumbItem, siteItems, fileExtensions, searchDrive, customStyle, spinnerStatus } = this.state;
    this._itemID = itemID;
    this._siteID = siteID;
    ...
    return (
      <div>
        ...
        <div>
          ...
          <Spinner open={spinnerStatus}></Spinner>
          {(this.state.itemID != "" && this.state.searchDrive == "") &&
            <div>
              <DragDropFiles
                ...
                labelMessage="Custom upload File"
              >
                <FileList
                  ref={this.flref}
          ...
          {(this.state.itemID != "" && this.state.searchDrive == "") &&
            <div>
              <input
                ...
                multiple={true} onChange={(event: React.ChangeEvent<HTMLInputElement>) => this._handleFileUploadExplorer(event)}
              />
              <DragDropFiles
                dropEffect="copy"
                enable={true}
  ...
  private _getDropFiles = async (files: any) => {
    ...
    const initialized = await graphService.initialize(this.props.serviceScope);

    if (initialized) {
      const tmpFileID = await graphService.uploadTmpFileToOneDrive(this.state.siteID, this.state.itemID, files);
    }
    this.flref.current.reload(true);
    this.setState({
      spinnerStatus: false
  ...
  private _handleFileUploadExplorer = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (!event || event.target.files.length < 1) {
      ...
      for (var i = 0; i < event.target.files.length; i++) {
        const file: any = event.target.files.item(i);
        file.fullPath = "";
        files.push(file);
      }
      this._getDropFiles(files);
    }
  }
   ...
   * 
   */
  private postGraphContent = (graphQuery: string, file) => {
